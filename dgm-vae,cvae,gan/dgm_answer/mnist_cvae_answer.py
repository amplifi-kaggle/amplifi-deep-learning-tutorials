...
hid_enc = conv(x_reshape, n_ch, [3, 3], strides=2, padding='SAME', activation=tf.nn.relu)
hid_enc = conv(hid_enc, 2*n_ch, [3, 3], strides=2, padding='SAME', activation=tf.nn.relu)
hid_enc = conv(hid_enc, 4*n_ch, [3, 3], strides=2, padding='VALID', activation=tf.nn.relu)
hid_enc = tf.reshape(hid_enc, [-1, 3*3*4*n_ch])
...
# define decoder
z = q_z.sample()
hid_dec = dense(z, 3*3*4*n_ch, activation=tf.nn.relu)
hid_dec = tf.reshape(hid_dec, [-1, 3, 3, 4*n_ch])
hid_dec = deconv(hid_dec, 2*n_ch, [3, 3], strides=2, padding='VALID', activation=tf.nn.relu)
hid_dec = deconv(hid_dec, n_ch, [3, 3], strides=2, padding='SAME', activation=tf.nn.relu)
logits = deconv(hid_dec, 1, [3, 3], strides=2, padding='SAME', activation=None)
