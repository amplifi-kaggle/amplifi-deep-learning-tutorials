...
def generate(noise, reuse=None):
    with tf.variable_scope('gen', reuse=reuse):
        hid = dense(noise, n_hid, activation=tf.nn.relu)
        x = dense(hid, 784, activation=tf.nn.sigmoid)
        return x

# discriminator network
def discriminate(x, reuse=None):
    with tf.variable_scope('dis', reuse=reuse):
        hid = dense(x, n_hid, activation=tf.nn.relu)
        logits = dense(hid, 1)
        return logits
...
real_logits = discriminate(real)
fake_logits = discriminate(fake, reuse=True)
