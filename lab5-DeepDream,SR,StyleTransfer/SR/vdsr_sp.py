...
def VDSR(x):
    ...
    x = Conv2D(x, [3, 3, 64, r*r], [1, 1, 1, 1], 'SAME', name='conv20')
...
def PrepareTrainImages():
    ...
    train_labels = np.zeros((len(train_filenames), H*r, W*r, C), dtype=np.uint8)
...
with tf.Session() as sess:
    ...
    if phase == 'Train':
        ...
        saver.restore(sess, './VDSR_models/VDSR_SP_iter100.ckpt')
    ...
    if phase == 'Train':
        ...
        for i in range(0, 100000):
            ...
            if i % 100 == 0:
                save_path = saver.save(sess, './VDSR_models/VDSR_SP_iter'+str(i)+'.ckpt')
    ...
    for i in range(0, num_test_samples):
        ...
        Image.fromarray(img_input[:,:,0].astype(np.uint8)).save(test_filenames[i]+'_sp_input.png')
        ...
        res.save(test_filenames[i]+'_VDSR_SP_x2.png')
