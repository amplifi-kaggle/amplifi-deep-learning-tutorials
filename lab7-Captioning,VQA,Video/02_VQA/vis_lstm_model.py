    ...
    def forward_pass_lstm(self, word_embeddings):
        ...
        for l in range(self.options['num_lstm_layers']):
            ...
            for lstm_step in range(self.options['lstm_steps']):
                if lstm_step == 0:
                    lstm_preactive = tf.matmul(x[lstm_step], self.lstm_W[l]) \
                                        + self.lstm_b[l]
                else:
                    lstm_preactive = tf.matmul(h[lstm_step-1], self.lstm_U[l]) \
                    + tf.matmul(x[lstm_step], self.lstm_W[l]) + self.lstm_b[l]
                i, f, o, new_c = tf.split(axis=1, num_or_size_splits=4, \
                    value=lstm_preactive)
                i = tf.nn.sigmoid(i)
                f = tf.nn.sigmoid(f)
                o = tf.nn.sigmoid(o)
                ...
                # h[lstm_step] = o * tf.nn.tanh(c[lstm_step]) # <-- Correct one, but let's just stick with the pre-trained model
                h[lstm_step] = o * tf.nn.tanh(new_c) # Wrong one, with a pre-trained model
